/**
 * Local-first API for Water Warrior (replaces Base44 SDK).
 * Data is stored in localStorage so the app runs without an external backend.
 */
...
function ensureSeedData() {
  ...
  if (users.length === 0) {
    writeJson(STORAGE.users, DEMO_USERS.map((u) => ({ ...u, created_date: new Date().toISOString() })));
  }
}
